...
class TelnetClient
{
     ...
     * @throws \Telnet\Exceptions\NameResolutionException on error
     * @throws \Telnet\Exceptions\ConnectionException on error
     */
     ...
     *
     * @return boolean
     * @throws \Telnet\Exceptions\UnlikelyException if closing the socket failed
     */
     ...
     * @return boolean
     * @throws \Telnet\Exceptions\LoginException on error
     */
    ...
    private function waitForNbData($length = null)
    {
        ...
        } elseif (!is_null($length) && (!is_int($length) || $length < 1)) {
    ...
    private function _getNextLine_cb($nbchar, $c, &$firstCharTs)
    {
        if (is_null($firstCharTs)) {
            ...
            return false;
        } elseif (!is_null($this->full_line_timeout) && $firstCharTs + $this->full_line_timeout <= microtime(true)) {
            return false;
        }
     ...
     * @throws \Telnet\Exceptions\ConnectionTimeoutException
     *
     ...
     * @throws \Telnet\Exceptions\UnimplementedException on unknown state
     */
     ...
     * @param array $a_c array of characters to process.
     * @return boolean true if more characters are needed, false if processing is done ($a_c was cleaned of TELNET protocol data such that it contains only actual data)
     * @throws \Telnet\Exceptions\ConnectionException if sending command negotiation fails
     */
     ...
     * @param boolean $add_newline Default true, adds newline to the command
     * @return boolean true on success
     * @throws \Telnet\Exceptions\ConnectionException if connection on socket errors
     */
